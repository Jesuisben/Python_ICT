# coffees = [] # empty list
coffees = list() # 함수를 이용하여 빈 리스트 생성

coffees.append('아메리카노')
coffees.append('콜드브루')
coffees.append('카푸치노')
coffees.append('바닐라라떼')
coffees.append('디카페인커피')
coffees.append('카페라떼')

count = len(coffees) # 요소 개수
print('요소 개수 : %d' % count)

# 인덱싱
print('앞에서 2번째 음료 : %s' % coffees[2])
print('뒤에서 1번째 음료 : %s' % coffees[-1])

# 슬라이싱
# coffees[start:end:step] : start부터 시작하여 end 직전까지 step 단계씩 슬라이싱하세요.

print('1번째부터 3번째 까지 요소들 : %s' % coffees[1:4])
print('3번째 이후 모든 요소들 : %s' % coffees[3:])
print('처음부터 2번째까지의 요소들 : %s' % coffees[:3])

print('전부 출력 : %s' % coffees[::])
print('짝수번째 요소들 출력 : %s' % coffees[0::2])
print('홀수번째 요소들 출력 : %s' % coffees[1::2])
print('색인이 3의 배수인 요소들 : %s' % coffees[0::3])

# 오름차순 정렬하기
print('오름차순 정렬')
coffees.sort()
print(coffees)

# 내림차순 정렬하기
print('내림차순 정렬')
coffees.sort(reverse=True)
print(coffees)

# 무작위로 섞기
import random
random.shuffle(coffees)
print(coffees)